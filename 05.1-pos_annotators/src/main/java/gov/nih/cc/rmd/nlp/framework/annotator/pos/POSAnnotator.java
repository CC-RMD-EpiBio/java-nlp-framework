...
public class POSAnnotator extends JCasAnnotator_ImplBase {
  ...
  public void process(JCas pJCas) throws AnalysisEngineProcessException {
    ...
    GLog.println(GLog.DEBUG_LEVEL, this.getClass(), "process","started POSAnnotator");
    ...
    GLog.println(GLog.DEBUG_LEVEL, this.getClass(), "process"," End POSAnnotator");
  ...
  private void posTag(JCas pJCas, Utterance pUtterance) throws AnalysisEngineProcessException {
    ...
    if ( termz != null ) {
      ...
      // these terms are backwords - sort by offset
      UIMAUtil.sortByOffset(terms);
    
      if ( terms != null && terms.size() > 0 )
        for ( Annotation term : terms) {
      
          FSArray tokenz = ((LexicalElement)term).getTokens();
          @SuppressWarnings("unchecked")
      ...
      if ( tokenArray != null && tokenArray.length > 0 )
        try {
        tags = this.tagger.tag( tokenArray); 
        } catch( Exception e) {
      ...
      if ( tags != null ) {
        for (int i = 0; i < tags.length; i++ ) {
          ...
          // --------------------------
          // The pen treebank has a funky thing about the "to" tag
          // it misses verbs that start with to _verb_  
          // --------------------------
          if ( lastTag != null && lastTag.contentEquals("TO")) {
            aPOS.setPos("VB");
          } else {
            aPOS.setPos( tags[i]);
          }
   ...
   */
  // -----------------------------------------
  private void setTermPOS(JCas pJCas, List<Annotation> pTerms) {
   
    if ( pTerms != null )
      for ( Annotation term : pTerms )
        setTermPOS( pJCas, (LexicalElement) term);
		...
		if (pTerm != null) {
			...
			if (tokens != null) {
				if (tokens.size() == 1)
					...
					if (posFeatureArray != null && posFeatureArray.size() == 1) { // < if (!pTerm.getAmbiguous()
					...
					} else if ( posFeatureArray != null && posFeatureArray.size()>0){
						// ----------------------------
						...
						for (int i = 0; i < posFeatureArray.size(); i++) {
						 ...
						 PartOfSpeech basicPOS= new PartOfSpeech(pJCas);
						 basicPOS.setBegin( pTerm.getBegin());
						 basicPOS.setEnd( pTerm.getEnd() );
						 basicPOS.setPos("noun");
						 basicPOS.addToIndexes();
						 pTerm.setPartOfSpeech(basicPOS);
