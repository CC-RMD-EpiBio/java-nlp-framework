...
public class DateByLookupAnnotator extends JCasAnnotator_ImplBase  {
  ...
  private final void absoluteDateLookup(JCas pJCas) {
    ...
    if ( tokens != null && !tokens.isEmpty()) {
      ...
      for ( Annotation token : tokens ) {
        String buff = token.getCoveredText();
   ...
   try {
     ...
     if ( !U.isOnlyPunctuation( buff ) ) {
       ...
       if ( cols != null ) {
         ...
         if (yearSeen ||( couldBeMonth && (couldBeDay || yearSeen ) && ctr >= 2))
   ...
   if (terms != null) {
     ...
     for (Annotation term : terms ) {
       try {
         ...
         if ( types != null &&  !types.contains("DateKind") && (types.contains("Date") || types.contains("Month")|| types.contains("Day") || types.contains("Holiday")))
  ...
  */
 // -----------------------------------------
  void createDate( JCas pJCas, Annotation pMention ) {
   
    
    boolean makeDate = false;
    
    List<Annotation> dates = null;
    int total = pJCas.getDocumentText().length();
    int begin = pMention.getBegin();
    int _end  = pMention.getEnd();
    // look to the left by a few chars to pick up dates before this token
    // look to the right by a few cars to pick up dates after this token
    if ( begin-5 > 0) begin = begin -5;
    if ( _end + 5  < total) _end = _end + 5;
    dates = UIMAUtil.fuzzyFindAnnotationsBySpan(pJCas, Date.typeIndexID,begin, _end, true );
  
    if ( dates != null && !dates.isEmpty())
      makeDate = datesCoverConceptRange( pMention, dates ) ;
    else
      makeDate = true;
    ...
    if ( makeDate ) {
      ...
      statement.setSemanticTypes("Date:PII" );
      statement.addToIndexes();  
...
// =================================================
  private final boolean datesCoverConceptRange(Annotation pConcept, List<Annotation> pDates) {
    ...
    if ( pDates != null && !pDates.isEmpty())  {
      ...
      int[]  maxSpans = null;
      try { maxSpans = UIMAUtil.getMaxSpans(pDates); } catch (Exception e) { returnVal = false; return returnVal ; }
