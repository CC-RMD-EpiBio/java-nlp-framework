...
public class AssertionAnnotator extends JCasAnnotator_ImplBase {
  ...
  public void process(JCas pJCas) throws AnalysisEngineProcessException {
    ...
    try {
      ...
      e.printStackTrace();
      GLog.println(GLog.ERROR_LEVEL,this.getClass(), "process", "Issue with the assertion annotator " + e.toString());
      // throw new AnalysisEngineProcessException();
    }
  ...
  */
  ...
  //  List<Annotation> sentences = UIMAUtil.getAnnotations(pJCas, Utterance.typeIndexID, true);
    ...
    for (Annotation someEvidence : pAllEvidences) {
     ...
     String buff = someEvidence.getCoveredText();
      ...
      if (triggerTypes == null) {
        ...
        for (String triggerType : triggerTypez) {
          ...
          markAnnotation(  someEvidence, someEvidence );
  ...
  private void markAnnotation(Annotation anAnnotation, Annotation someEvidence) {
    ...
    String subject = getSubjectFor( anAnnotation);
    if ( subject == null )
  ...
  // =================================================
  /**
   * getSubjectFor retrieves subject for a concept or lexical element
   * 
   * @param anAnnotation
   * @return String
  */
  // =================================================
   private final String getSubjectFor(Annotation anAnnotation) {
    String returnVal = null;
    
    try {
     returnVal = ((Concept) anAnnotation).getSubjectStatus();

    } catch (Exception e) {
      try {
       returnVal = ((VAnnotation) anAnnotation).getSubject();
