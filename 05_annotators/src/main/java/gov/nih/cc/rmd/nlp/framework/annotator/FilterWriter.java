 ...
 public void destroy() {
  ...
  public void initialize(String [] pArgs ) throws ResourceInitializationException {
    ...
    if (  pArgs != null && pArgs.length > 0 ) {
     ...
     String filterOutNegationz = U.getOption(pArgs, "--filterOutNegations=",  "false");
     filterOutNegation = Boolean.valueOf( filterOutNegationz);
   }
    ...
    this.filterOutNegation = pFilterOutNegation;
	  ...
	  if ( pOutputTypes != null && pOutputTypes.length > 0 ) {
		  ...
		  for ( String outputType : pOutputTypes) {
		    try {
		      if ( outputType != null ) {
		      String label = U.getNameWithoutNameSpace( outputType.trim());
		     
		      this.labelHash.add( label.trim());
